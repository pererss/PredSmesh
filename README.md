...
### Шаг 7. Регион (важно для скорости)

Supabase-база находится в **eu-west-1 (Ирландия)**, поэтому сервис Railway должен быть
в **EU West (Амстердам)** — иначе каждый запрос к базе идёт через океан (+150 мс) и бот
заметно тормозит.

Проверить/сменить регион:

- В дашборде: сервис → **Settings** → **Region** → **EU West**.
- Или через CLI: `railway service scale eu-west=1 sfo=0` (регион `sfo` = US West).

Замер для сравнения: подключение к базе из US West ~4 с, из EU West ~1 с.
